...
import threading  # To run notifications in the background
...
def check_notifications():
    ...
    for med in medicines:
        times_list = med[5].split(",")  # Get times for the medicine
        for time in times_list:
            if time.strip() == now:
                notification.notify(
                    title="Medicine Reminder",
                    message=f"It's time to take {med[1]} ({med[2]})",
                    timeout=10  # Notification stays for 10 seconds
                )
    # Schedule the function to run again in 1 minute
    threading.Timer(60, check_notifications).start()
...
# Start the notification checker in a separate thread
